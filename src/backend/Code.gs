 ...
 * Routes to different HTML pages based on URL parameter
 *
 * Usage:
 *   ?page=display  - Display controller (cycles through pages)
...
function doGet(e) {
  ...
  const page = (e.parameter.page || 'controller').toLowerCase();

  try {
    switch(page) {
      case 'controller':
      case 'display':
        // Display controller - cycles through pages
        return HtmlService.createTemplateFromFile('Display')
          .evaluate()
          .setTitle('The Wall - Display Controller')
...
function openDisplayController() {
  const url = ScriptApp.getService().getUrl() + '?page=controller';
  const html = `
    <html>
      <body>
        ...
        <p style="font-size: 12px; color: #666; margin-top: 10px;">
          The Display Controller allows you to cycle through all pages.
        </p>
        <script>
  ...
  SpreadsheetApp.getUi().showModalDialog(
    HtmlService.createHtmlOutput(html).setWidth(400).setHeight(180),
    'Display Controller'
  );
}
...
function openIntro() {
  const url = ScriptApp.getService().getUrl() + '?page=intro';
  const html = `
    <html>
      <body>
        <p>Open this URL in a new tab:</p>
